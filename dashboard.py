...
def create_monthly_orders_df(df):
    # Gunakan rule='ME' untuk agregasi bulanan
    monthly_orders_df = df.resample(rule='ME', on='order_purchase_timestamp').agg({
        "order_id": "nunique",
        "payment_value": "sum"
    })
    # Mengubah format index menjadi nama bulan dan tahun agar rapi di grafik
...
with st.sidebar:
    ...
    try:
        ...
        if len(date_range) == 2:
            ...
            st.error("Silakan pilih rentang waktu mulai Januari 2017 hingga Oktober 2018.")
            st.stop()
            
    except Exception as e:
        st.error(f"Terjadi kesalahan dalam pemilihan tanggal: {e}")
        st.stop()

# Filter data berdasarkan rentang yang valid
...
# 3. Best Customer Based on RFM Parameters
...
st.caption('Copyright (c) Tsabita Ransi Aurelia 2026')
